...
/**
 * Test suite for Kruskal algorithm (edge sorting + DSU).
...
public class KruskalTest {
    ...
    public void testTriangleWithUniqueMinimum() {
        ...
        Result result = Kruskal.computeMST(graph, metrics);
    ...
    @Test
    @DisplayName("Edges already sorted by weight")
    public void testPreSortedEdges() {
        List<String> nodes = Arrays.asList("A", "B", "C", "D");
        List<Edge> edges = Arrays.asList(
            new Edge("A", "B", 1),
            new Edge("B", "C", 2),
        ...
        assertEquals(6, result.getTotalCost(), "Linear path: 1+2+3=6");
        assertEquals(3, result.getMstEdges().size());

        System.out.println("Pre-sorted edges: " + metrics.get() + " operations");
    }

    @Test
    @DisplayName("Cycle detection - reject edge that creates cycle")
    public void testCycleDetection() {
        List<String> nodes = Arrays.asList("A", "B", "C");
        List<Edge> edges = Arrays.asList(
            new Edge("A", "B", 1),
            new Edge("B", "C", 2),
            new Edge("A", "C", 10)  // This will create cycle, should be rejected
        );
        Graph graph = new Graph(nodes, edges);

        Result result = Kruskal.computeMST(graph, metrics);

        assertEquals(3, result.getTotalCost(), "Should not include A-C edge");
        assertEquals(2, result.getMstEdges().size(), "Only 2 edges in MST");

        // Verify the edge A-C with weight 10 was not included
        boolean hasHeavyEdge = result.getMstEdges().stream()
            .anyMatch(e -> e.w == 10);
        assertFalse(hasHeavyEdge, "Heavy edge creating cycle should be rejected");
    }
}
